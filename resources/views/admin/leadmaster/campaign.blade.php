...
@section('dashboard_section')
    <div>
        <nav class="bg-info d-flex justify-content-between border border-dark" aria-label="breadcrumb">
            ...
            <div class="alert alert-success m-0 d-flex align-items-center" id="flashMsg" role="alert"
                style="width: 0px;height:51px;position:fixed;right:0;transition:width 1s;border:0;padding:0rem;"></div>
                        ...
                        @foreach ($campaign as $key => $item)
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        <button onclick="editFormData(event,'{{ json_encode($item) }}')" type="button"
                                            title="MODIFY" class="btn btn-warning btn-sm m-1"><i
                                                class="fa fa-pencil-square-o" aria-hidden="true"></i></button>
                                        <a href="{{ route('deleteCampaign', ['id'=>$item->id]) }}" title="DELETE"
                                            class="btn btn-danger btn-sm m-1"><i class="fa fa-trash-o"
        ...
        function callToActiveModal(getData) {
            ...
            let html = ` <div class="modal fade" id="updateItems" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
                        aria-labelledby="staticBackdropLabel" aria-hidden="true">
                            ...
                            <form class="" id="campaignForm" onsubmit="return postItem(event,${obj.id})">
